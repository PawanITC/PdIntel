@relay
Feature: Outbox relay — Stripe events to Kafka

  As the Plany payment service
  I want the outbox relay to publish pending Stripe events to Kafka
  ...
  @smoke
  Scenario: PENDING outbox row is published to Kafka and marked PUBLISHED
    ...
    Then a Kafka message is published to topic "plany.stripe.webhook-raw.v1"
  ...
  Scenario: Kafka message payload contains the original Stripe event
